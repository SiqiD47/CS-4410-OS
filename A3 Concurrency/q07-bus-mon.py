...
class BusStoplight(MP):
    ...
    def cross(self, direction):
        ...
        # TODO implement me
        with self.lock:
            while (self.departingStudents.read() > 0 and direction == 0) or (self.boardingStudents.read() > 0 and direction == 1):
                if self.departingStudents.read() > 0 and direction == 0:
                    self.canBoard.wait()
                if self.boardingStudents.read() > 0 and direction == 1:
                    self.canDepart.wait()
            if direction == 0:
